...
### Sesión del 10 de Junio de 2026 (Sesión 26) — `next/image` (reducir peso real de imágenes) + fix `sharp` en producción

#### Contexto / objetivo
Un test de [GTmetrix](https://gtmetrix.com) sobre `afroeventos.com` (tras los cambios de la Sesión 25) arrojó: grade **C**, Performance **67%**, Structure 91%, LCP **3.0 s**, TBT 0 ms, CLS 0.06, Page size **811 KB** (IMG **525 KB** = 65%), 38 requests. El issue principal: *"Properly size images" — ahorro potencial 465 KB*: se servían imágenes de 1080px en tarjetas de ~300px. El lazy loading de la Sesión 25 retrasa la descarga pero **no reduce el peso**. Decisión del usuario: **Opción A — `next/image`** (redimensiona + formato moderno automáticamente).

#### Cambios realizados

**1. Migración a `next/image` con `fill` + `sizes` (componentes de la home)**
- `EventCard.tsx`, `HeroCarousel.tsx` (`priority` en el slide activo), `FeaturedCarousel.tsx`, `FeaturedEventsSection.tsx` (caso `featured-card-v` de 2 eventos), `BannerSlider.tsx`.
- Se usa `fill` porque todos esos contenedores son `position:relative` + `aspect-ratio` + `overflow:hidden`; el `object-fit:cover` vive en el CSS y se preserva sobre el `<img>` que genera `next/image`.
- **Imágenes dejadas como `<img>` lazy a propósito**: hero del detalle (`event-detail-hero-banner`), galería (`evg-main-img`, single), mapa de asientos, y `featured-card-h` (1 evento). Todas son `height:auto` de **ratio natural** ("flyer completo") donde `fill` recortaría/distorsionaría; ahí la imagen se muestra grande, no sobredimensionada.
...
**4. Fix `sharp` en el Dockerfile del web-client (crítico para producción)**
- El trazado de Next al standalone copia `sharp/lib` pero **omite el binario nativo + las libs de libvips** (`@img/sharp-libvips-*`). Síntoma: el optimizador **no falla**, sino que **degrada a servir el original sin redimensionar** (status 200, mismo peso) → se pierde todo el ahorro.
- Fix: en el stage `runner` se copia desde el `builder` el `@img` + `sharp` completos al `node_modules` del standalone:
  ```dockerfile
...
#### Verificación
- `nx build web-client` (exit 0). El HTML genera `srcset` con `/_next/image`.
- Optimizador (en `next dev` y en standalone con los binarios): tarjeta `w=384` → **42.728 bytes (384×512)** vs original **110.636 bytes (600×800)** = **−61%**.
- Diagnóstico del bug de `sharp`: `require('sharp')` en el standalone fallaba con `ERR_DLOPEN_FAILED` (faltaban `libvips-*.dll` / `.so` junto al `.node`).
- **Build Docker real (alpine) verificado**: imagen construida (exit 0, 351 MB); dentro del contenedor `@img/sharp-linuxmusl-x64.node` + `libvips-cpp.so` presentes; `sharp 0.34.5` carga y redimensiona correctamente.

#### Cómo queda el flujo de imágenes
- **Subida** (ya existía): Sharp → WebP + compresión, se guarda en `api.afroeventos.com/uploads/...`.
...
### Sesión del 10 de Junio de 2026 (Sesión 25) — Performance (Pingdom) + Cierre de Deuda Técnica

#### Contexto / objetivo
Un test de [Pingdom](https://tools.pingdom.com) sobre `afroeventos.com` arrojó: Performance grade **B 89**, Page size **912.8 KB**, Load time **5.24 s**, **25 requests**. Las tres recomendaciones principales (todas grade C): *Make fewer HTTP requests*, *Compress components with gzip*, *Add Expires headers*. Objetivo: atacar las que dependen de configuración del servidor y, de paso, cerrar la deuda técnica de archivos huérfanos.

#### Estado previo de la deuda técnica (verificado en código)
- **Límite de 3 banners en backend** → ya estaba resuelto en `banners.service.ts` (`create()` valida `count >= 3` y lanza `BadRequestException`).
- **BannerSlider sin animación** → ya estaba resuelto en `global.css` (`.banner-slide { opacity:0; transition: opacity 0.6s ease }` + `.banner-slide--active { opacity:1 }`).
- **Archivos huérfanos en disco** → **pendiente** → resuelto en esta sesión (ver abajo).

#### Cambios realizados

**1. Expires / Cache headers en `/uploads` (`apps/api/src/app/app.module.ts`)**
- `ServeStaticModule.forRoot()` ahora incluye `serveStaticOptions: { maxAge: '365d', immutable: true }`.
- Las imágenes responden con `Cache-Control: public, max-age=31536000, immutable`. Seguro porque los nombres son UUID: al reemplazar un logo/avatar se genera un UUID nuevo (URL nueva), nunca se sobrescribe el contenido de una URL existente.
- **Efecto:** en visitas repetidas el navegador deja de re-descargar las imágenes (la mayor parte de los 912 KB). Ataca *Add Expires headers*.

**2. Compresión gzip en el API (`apps/api/src/main.ts`)**
- `app.use(compression())` (paquete `compression` añadido a `dependencies`, `@types/compression` a `devDependencies`).
- Import vía `require` (igual que `sharp`) porque `esModuleInterop` está desactivado.
- Comprime JSON de la API y SVG (~70%). Las imágenes WebP ya están comprimidas y el middleware las omite por content-type. Ataca *Compress components with gzip*.
- Nota: el web-client (Next.js standalone) **ya** comprime su HTML/JS/CSS y cachea `/_next/static` con `immutable` por defecto; el cuello de botella era el API.

**3. Lazy loading nativo en el web-client (12 `<img>` en 7 archivos)**
- `EventCard.tsx`, `FeaturedEventsSection.tsx` (×2), `FeaturedCarousel.tsx`, `BannerSlider.tsx` (×2), `my-profile/page.tsx` → `loading="lazy"` + `decoding="async"`.
- `HeroCarousel.tsx` → above-the-fold: card activa `loading="eager"` + `fetchPriority="high"`; resto `loading="lazy"`. Evita penalizar el LCP.
- `EventDetailClient.tsx` → hero del evento (banner desktop + cuadrada móvil): **ambos `loading="lazy"`** (el oculto por `display:none` según media query nunca entra al viewport → se elimina el doble-fetch que existía); banner con `fetchPriority="high"`. Galería, logo del organizador y mapa de asientos → `loading="lazy"`.
- **Efecto:** las imágenes below-the-fold no se descargan hasta hacer scroll (menos requests iniciales, mejor LCP). Ataca parcialmente *Make fewer HTTP requests*.
- ⚠️ **No reduce el peso por imagen** (siguen siendo 1080px en tarjetas pequeñas). Pendiente futuro: `next/image` o miniaturas en backend.

**4. Deuda técnica — limpieza de imágenes huérfanas (`apps/api/src/app/scheduler/orphaned-uploads.scheduler.ts`, nuevo)**
- `OrphanedUploadsScheduler` con `@Cron(EVERY_DAY_AT_3AM)`. Registrado en `scheduler.module.ts`.
- Recolecta todos los basenames referenciados en BD (events ×6 campos + `galleryUrls`, `User.avatarUrl`, `OrganizerMember.avatarUrl`, `OrganizerProfile.organizationLogo`, `Banner.imageUrl`).
- Recorre `uploads/` y borra los archivos **no referenciados** y con **más de 24h** (periodo de gracia: no borra uploads recién subidos que esperan a que se guarde el formulario).
- No corre si `STORAGE_PROVIDER=cloudinary`. Loguea cuántos borró y cuántos MB liberó.
- Matching por basename (UUID único). Complementa el `deleteOrphanedImageFiles()` de `events.service.ts`, que solo limpia al editar/borrar un evento existente.

#### Verificación
- Typecheck API (exit 0) + build de producción API (webpack OK) + `next build` web-client (exit 0).
- API levantado con `node dist/apps/api/main.js`: gzip confirmado (`Content-Encoding: gzip` + `Vary: Accept-Encoding` con Accept-Encoding; sin comprimir sin él); cache confirmado (`Cache-Control: public, max-age=31536000, immutable` en imagen real).
- Web-client con `next start`: HTML de la home renderiza 9 `<img>` → 1 `eager`+`fetchPriority=high` (hero activo), 6 `lazy`, todos `decoding=async`; React 19 además inyecta un `<link rel=preload>` para el hero de alta prioridad.

#### Pendiente / próximos pasos
- Reducir peso real por imagen: `next/image` (requiere `remotePatterns` + `sharp` en el Dockerfile runner + volumen de caché en Coolify) **o** generar miniaturas en el backend (que ya usa Sharp). Decisión del usuario: empezar por lazy loading (hecho); evaluar el siguiente paso después.
