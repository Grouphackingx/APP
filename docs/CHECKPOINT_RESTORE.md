...
**Fecha de Última Actualización:** 9 de Junio de 2026 (Sesión 23)
...
- **Favicons (web-client)**: `public/favicon.svg` negro con `rx="120"` (esquinas redondeadas). `public/logo-og.svg` para JSON-LD Google. `public/apple-touch-icon.png` (1080×1080) para iOS y mensajería. `public/favicon.ico` eliminado — redirect 301 en `next.config.js`. `opengraph-image.tsx` en `src/app/` genera la imagen de preview social (1200×630). Web-host y web-admin siguen usando sus propios SVG via `<link>` en JSX.
