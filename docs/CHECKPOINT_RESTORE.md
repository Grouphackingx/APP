...
**Fecha de Última Actualización:** 10 de Junio de 2026 (Sesión 26)
**Estado del Proyecto:** COMPLETO Y EN PRODUCCIÓN — Fases 1-4 + Portal Cliente completo + Panel Host completo + Panel Admin completo + Sistema de Emails Transaccionales completo + Auth flow (verify/forgot/reset password) + URLs `/eventos/` en español + Favicons AfroEventos + Sistema de Banners Publicitarios full-stack + UI/UX Portal Cliente (Destacados Adaptativos + FeaturedCarousel + EventsGrid paginación real) + OrganizerCTA + Navbar dropdown + Galería rediseñada + sellOnSite en zonas (full-stack) + Bloqueo de Organizadores (full-stack, sesión inmediata) + Modales personalizados (sin confirm/alert nativo) + Persistencia de vista en URL + Impersonación de Organizadores por Admin + Control de Pasarela de Pagos (global + por org) + Límite anual por aniversario en planes + Paginación real en API + Sistema de imágenes optimizado (Sharp WebP, límites y formatos configurables desde .env) + **UI Polish**: precios ocultos en EventCard, hover shadows navbar eliminados, logos sidebars y footer clicables + **TODOS LOS SERVICIOS EN PRODUCCIÓN**: API + 3 frontends (Coolify) + DB schema aplicado + admin "Blade" creado + **EMAILS EN PRODUCCIÓN**: Resend SMTP (puerto 587) + 12 plantillas con logo oficial AfroEventos + best practices Gmail/Outlook + **Sesión 14**: Sistema de Categorías de Eventos full-stack + **Sesión 15**: Modo Prueba + Kill-switch pagos + UX compra + **Sesión 16**: `.env` sacado de git + `findAll` optimizado + limpieza imágenes huérfanas + `as any` eliminados + **Sesión 17**: UX Responsive completo + **Sesión 18**: Validación formularios completa + **Sesión 19**: deuda técnica `Ticket.eventId` + Sistema imágenes resize server-side + **Sesión 20**: Rate limiting API + **Sesión 21**: Copy UI web-client (hero headline "EL PUNTO DE ENCUENTRO DE NUESTRA GENTE", subtítulo, tipografía) + web-admin puerto 4202 fijado + **Sesión 22**: SEO/OG metadata completo + opengraph-image.tsx + favicon negro rx=120 + apple-touch-icon.png + JSON-LD Organization + redirect /favicon.ico + **Sesión 23**: Email de alerta a admins globales al registrarse un nuevo organizador (template `new-organizer-registered`, `sendNewOrganizerAlert`, fire-and-forget en `registerHost`) + **Sesión 24**: Pantalla PENDING dedicada sin sidebar + authLoading guard (sin flash) + favicons corregidos en web-host/web-admin (SVG negro rx=120 + apple-touch-icon.png + icons config completo) + copy registro organizador + email welcome-host sin tiempos específicos + **Sesión 25**: Performance (test Pingdom) — cache headers Expires 1 año + immutable en `/uploads`, compresión gzip en API (`compression`), lazy loading nativo en 12 `<img>` del web-client (hero `eager`+`fetchPriority=high`, resto `lazy`) + deuda técnica cerrada: cron `OrphanedUploadsScheduler` (limpieza diaria de imágenes huérfanas) + **Sesión 26**: `next/image` (Opción A) en componentes de la home (EventCard, HeroCarousel, FeaturedCarousel, featured-card-v, BannerSlider) con `fill`+`sizes` → redimensiona al tamaño real (tarjeta 42 KB vs 108 KB, −61%, ataca los 465 KB de "Properly size images" de GTmetrix); `remotePatterns` + `dangerouslyAllowLocalIP` solo si la API es local; fix Dockerfile web-client (copia `@img`+`sharp` al standalone, Next omite el binario nativo → optimizador degradaba a servir el original), verificado en contenedor alpine real; imágenes height:auto de ratio natural (hero detalle/galería/mapa/destacado 1 evento) se quedan como `<img>` lazy a propósito + **fix de calidad** (reportado por el usuario): las subidas vienen en WebP q80 y `next/image` re-comprimía en su default q75 (por debajo del origen) → `images.qualities: [75, 90]` + `quality={90}` en los 5 componentes; `sizes` corregidos porque pedían variantes más pequeñas que el tamaño mostrado (EventCard 280→380, Hero 420→500, Featured 360→400)
...
| Registro             | OK     | Subtítulo "Crea, administra y publica tus eventos"; planes con "por año" |
...
| **Cuenta en Revisión** | OK   | Pantalla full-page dedicada sin sidebar: badge PENDING, tracker 3 pasos, botón logout. `authLoading` guard elimina flash del dashboard al recargar |
...
- **Favicons (las 3 apps)**: `public/favicon.svg` negro con `rx="120"` (esquinas redondeadas) — **idéntico en web-client, web-host y web-admin** desde Sesión 24. `public/apple-touch-icon.png` (1080×1080) también en los 3. `icons` en cada `layout.tsx` declara array con SVG (`type: 'image/svg+xml'`), PNG con tamaño, `shortcut` y `apple`. Web-client además: `public/logo-og.svg` para JSON-LD Google, `public/favicon.ico` eliminado (redirect 301 en `next.config.js`), `opengraph-image.tsx` genera la imagen de preview social (1200×630).
...
- ~~**Límite de 3 banners no validado en backend**~~ ✅ **Resuelto**: `BannersService.create()` valida `count >= 3` y lanza `BadRequestException('Se permite un máximo de 3 banners.')`.
- ~~**BannerSlider sin transición entre slides**~~ ✅ **Resuelto**: `global.css` aplica fade (`.banner-slide { opacity:0; transition: opacity 0.6s ease }` + `.banner-slide--active { opacity:1 }`); el slider apila las slides con `position:absolute`.
- ~~**Archivos subidos pero no guardados quedan huérfanos en disco**~~ ✅ **Resuelto (10 Jun 2026, Sesión 25)**: `OrphanedUploadsScheduler` (`@Cron(EVERY_DAY_AT_3AM)`) recolecta los basenames referenciados en BD y borra de `uploads/` los archivos sin referencia con +24h de antigüedad (periodo de gracia). No corre con Cloudinary. Complementa el `deleteOrphanedImageFiles()` que limpia al editar/borrar eventos.
...
- **Peso de imágenes sin reducir por tamaño de display**: las tarjetas/listados sirven el WebP a tamaño completo (1080px) aunque se muestren pequeñas. La Sesión 25 aplicó lazy loading (difiere/prioriza la carga) pero no reduce KB por imagen. Mejora futura: `next/image` (requiere `remotePatterns` + `sharp` en Dockerfile + volumen de caché en Coolify) o miniaturas en el backend.
